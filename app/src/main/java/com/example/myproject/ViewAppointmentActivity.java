...
public class ViewAppointmentActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        appointmentsListView.setOnItemClickListener((parent, view, position, id) -> {
            ...
            if (!selectedAppointment.isBooked()) {
                markAppointmentAsBooked(selectedAppointment.getId());
            } else {
                Toast.makeText(ViewAppointmentActivity.this, "This slot is already booked", Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    void markAppointmentAsBooked(String slotId) {
        ...
        if (currentUser != null) {
            String doctorId = currentUser.getUid();
            databaseReference.child(doctorId).child(slotId).child("BookedSlots").setValue(true)
                    .addOnSuccessListener(aVoid -> {
                        Toast.makeText(ViewAppointmentActivity.this, "Appointment marked as booked", Toast.LENGTH_SHORT).show();
                        fetchAppointments();
                    })
